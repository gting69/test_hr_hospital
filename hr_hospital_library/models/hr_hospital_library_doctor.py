...
class HrHospitalLibraryDoctor(models.Model):
    ...
    def _compute_display_name(self):
        for rec in self:
            ...
            if rec.specialty_id:
                rec.display_name = f"{name} ({rec.specialty_id.name})"
            else:
    ...
    def _onchange_is_intern(self):
        if self.is_intern and not self.mentor_id:
            mentor = self.env['hr.hospital.library.doctor'].search(
                [('is_intern', '=', False)],
                limit=1,
            )
    ...
    def _compute_experience(self):
        ...
        for rec in self:
            if rec.license_date:
                rec.experience = (
                    today.year - rec.license_date.year - (
                        (today.month, today.day) <
                        (rec.license_date.month, rec.license_date.day)
                    )
                )
            else:
                rec.experience = 0
