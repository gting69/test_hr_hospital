...
class MassReassignDoctorWizard(models.TransientModel):
    ...
    old_doctor_id = fields.Many2one('hr.hospital.library.doctor', string='Old Doctor')
    new_doctor_id = fields.Many2one('hr.hospital.library.doctor', string='New Doctor', required=True)
    patient_ids = fields.Many2many('hr.hospital.library.patient', string='Patients', required=True)
...
class RescheduleVisitWizard(models.TransientModel):
    ...
    visit_id = fields.Many2one('hr.hospital.library.visit', string='Current Visit', readonly=True)
    new_doctor_id = fields.Many2one('hr.hospital.library.doctor', string='New Doctor')
    new_date = fields.Date(string='New Date', required=True)
    new_time = fields.Float(string='New Time', required=True)
    reason = fields.Text(string='Reason for Reschedule', required=True)

    def action_reschedule(self):
        self.visit_id.state = 'cancelled'

        new_datetime = fields.Datetime.to_datetime(self.new_date) + timedelta(hours=self.new_time)

        self.env['hr.hospital.library.visit'].create({
            'name': f"Rescheduled: {self.visit_id.name}",
...
class DiseaseReportWizard(models.TransientModel):
    ...
    doctor_ids = fields.Many2many('hr.hospital.library.doctor', string='Doctors')
    disease_ids = fields.Many2many('hr.hospital.disease.type', string='Diseases')
    country_ids = fields.Many2many('res.country', string='Countries')
    start_date = fields.Date(string='Start Date', required=True)
    end_date = fields.Date(string='End Date', required=True)
    report_type = fields.Selection([('detail', 'Detailed'), ('summary', 'Summary')], default='detail')
    group_by = fields.Selection([('doctor', 'By Doctor'), ('disease', 'By Disease')], string='Group By')

    def action_generate_report(self):
        self.ensure_one()
        domain = [('visit_id.planned_datetime', '>=', self.start_date),
                  ('visit_id.planned_datetime', '<=', self.end_date)]
        if self.doctor_ids: domain.append(('visit_id.doctor_id', 'in', self.doctor_ids.ids))
        if self.disease_ids: domain.append(('disease_id', 'in', self.disease_ids.ids))

        return {
            'name': 'Disease Analysis Result',
            'type': 'ir.actions.act_window',
            'res_model': 'medical.diagnosis',
            'view_mode': 'tree,form',
            'domain': domain,
            'context': {'group_by': self.group_by or 'disease_id'},
        }
...
class DoctorScheduleWizard(models.TransientModel):
    ...
    start_week = fields.Date(string='Start Week', required=True)
    weeks_count = fields.Integer(string='Weeks Count', default=1, required=True)
    monday = fields.Boolean('Mon')
    tuesday = fields.Boolean('Tue')
    wednesday = fields.Boolean('Wed')
    thursday = fields.Boolean('Thu')
    friday = fields.Boolean('Fri')
    saturday = fields.Boolean('Sat')
    sunday = fields.Boolean('Sun')
    start_hour = fields.Float(string='Start Hour')
    end_hour = fields.Float(string='End Hour')

    def action_generate_schedule(self):
        self.ensure_one()
        days_map = {'0': self.monday, '1': self.tuesday, '2': self.wednesday, '3': self.thursday, '4': self.friday,
                    '5': self.saturday, '6': self.sunday}
        ...
        for week in range(self.weeks_count):
            for day_code in selected_days:
                schedule_date = self.start_week + timedelta(weeks=week, days=int(day_code))
...
class PatientCardExportWizard(models.TransientModel):
    ...
    export_format = fields.Selection([('json', 'JSON'), ('csv', 'CSV')], string='Export Format', default='json',
                                     required=True)
